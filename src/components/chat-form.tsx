...
export function ChatForm({ className, ...props }: React.ComponentProps<"form">) {
    ...
    <div className="my-4 flex h-fit min-h-full flex-col gap-4">
      {messages.map((message, index) => (
        <div
          ...
          className="max-w-screen rounded-xl px-3 py-2 text-sm data-[role=assistant]:self-start data-[role=user]:self-end data-[role=assistant]:bg-gray-100 data-[role=user]:bg-blue-500 data-[role=assistant]:text-black data-[role=user]:text-white"
        >
          {message.content}
        </div>
      ))}
  ...
  return (
    <main
      className={cn(
        "ring-none mx-auto flex h-svh max-h-svh w-full max-w-screen flex-col items-stretch border-none px-36 pt-12",
        className,
      )}
      {...props}
    >
